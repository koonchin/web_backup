...
class Web():
    ...
    def send_sales_report(self,dep,date=''):
        ...
        if dep == 'muslin':
            ...
            params = {'method': "GETORDERS", 'version': '3','orderdatebefore':date,'orderdateafter':date,'saleschannel':'facebook'}
            res = requests.get(url=url, headers=header, params=params)
            data = res.json()
            for customer in data['list']:
                if customer['status'].lower() != 'voided':
            ...
            params = {'method': "GETORDERS", 'version': '3','orderdatebefore':date,'orderdateafter':date,'saleschannel':'LINE'}
            res = requests.get(url=url, headers=header, params=params)
            data = res.json()
            for customer in data['list']:
                if customer['status'].lower() != 'voided':
            ...
            params = {'method': "GETORDERS", 'version': '3','orderdatebefore':date,'orderdateafter':date,'saleschannel':'IG'}
            res = requests.get(url=url, headers=header, params=params)
            data = res.json()
            for customer in data['list']:
                if customer['status'].lower() != 'voided':
...
def auto_send_8am():
    ...
    if datetime.datetime.now().strftime("%d%m%Y") == "06122023":
