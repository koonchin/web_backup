    ...
    def get_order_and_sku_amount(self,number):
        ...
        for i in data['list']:
            for product in i['list']:
                ...
                payload = {}
                payload['stocks'] = payload_list
                self.increase_stock(payload,'W0001')
...
def get_data(path,dep):
    ...
    if dep == 'muslin':
        column = {"sku":"SKU.1","name":"Description","amount":'Incoming ',"cost":'Unnamed: 13'}
    else:
...
def QC(dep,path):
    ...
    old_SKU = tuple([df.loc[i,'รหัสสต็อก'] for i in df.index])
    ...
    for i in range(len(old_SKU)):
        with open ("multi.txt","a",encoding="utf-8") as f:
            f.write(f"""insert into muslin.data_size values ("{old_SKU[i]}","{old_SKU[i].split("-")[1]}","Q{get_idsell(old_SKU[i])}",'{data_dict[old_SKU[i]]}');\n""")
# path = 'stock latest.xlsx'
# QC('muslin',path)
...
def Korkai(filename):
    ...
    insert_task = ', '.join([str(i) for i in cost_task])
...
access_tok = 'EAADs69tGsMEBO2GbuZCs99If8bs2wlMQTsdME64nRafp5342xWq6wFTszYUkL1rtNjHTgrO1vAvQKAS46mI0oFST2oMpLa1LDdaWoJVhkhqDvjGSoOTChMkkqxihSgtmTK488ZA4LSIBgWso02Lo1OZAgZCW9YYM252QzHpmIZBocYkgZCAJYz5wZDZD'
...
def transfer_all_fromwarehouse_by_idsell(path):
    df = pd.read_excel(path)
...
def setZeroReturnWarehouse():
    dep = 'muslin'
    web = Web(get_api_register(dep,'apikey'),get_api_register(dep,'apisecret'),get_api_register(dep,'storename'))
    web.set_zero_live_warehouse('W0005')
