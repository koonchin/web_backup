        ...
        if len(sku) == 2000:
            ...
            if len(sku) == 2000:
                ...
                if len(sku) == 2000:
                    ...
                    self.update(sku,amt,name,image,cost,price)
                    if len(sku) == 2000:
                        sku = self.web.get('GETPRODUCTS','sku',5)
                        amt = self.web.get('GETPRODUCTS','availablestock',5)
                        image = self.web.get("GETPRODUCTS",'imagepath',5)
                        cost = self.web.get("GETPRODUCTS",'purchaseprice',5)
                        name = self.web.get("GETPRODUCTS",'name',5)
                        price = self.web.get("GETPRODUCTS",'sellprice',5)
                        self.update(sku,amt,name,image,cost,price)
...
def set_three(dep):
    if dep == 'muslin':
        ...
        for i in range(len(sku)):
            ...
            taskdb_stock_main += f"WHEN '{sku[i]}' THEN {stock_amount}\n"
