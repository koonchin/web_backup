   ...
   # try:
    ...
    for i in get_database():
        try:
            ...
            set_three(i)
        # result = db.query_custom(f"select stock.sku,stock.amount + stock_main.amount from stock_main inner join stock on stock.sku = stock_main.sku",i) 
        # result = list(result.fetchall())
        # for i in result:
        #     if i[1] < 2 :
        #         send_line(f"{i[0]} - > ใกล้หมด")
            print('update')
        #update.get_track()
